...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    if [ ! -d "${odir}/urgent2025_challenge" ]; then
        log "Cloning https://github.com/urgent-challenge/urgent2025_challenge to ${odir}/urgent2025_challenge"
        git clone https://github.com/urgent-challenge/urgent2025_challenge "${odir}/urgent2025_challenge"
        cd "${odir}/urgent2025_challenge"
        git submodule update --init --recursive
        cd "${cwd}"
    fi

    python -m pip install -r "${odir}/urgent2025_challenge/requirements.txt"
fi
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    cd "${cwd}"
    cp -r "${odir}"/urgent2025_challenge/data "${PWD}/data"
fi
