...
inf_nums=   # To specify which data to use in inference for dataset with variable number of sources
...
# Evaluation related
scoring_protocol="NBPESQ ESTOI STOI SDR SAR SIR SI_SNR"
ref_channel=0
    ...
    if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
        ...
        for task in enh; do
            for use_true_nspk in false; do
                ...
                _dir="${enh_exp}/${inference_tag}/${task}"
                ...
                if [[ "${task}" == "tse" ]]; then
                    ...
                    infer_module=espnet2.bin.enh_veriable_number_speakers_inference_libricss
                fi
                # shellcheck disable=SC2046,SC2086
