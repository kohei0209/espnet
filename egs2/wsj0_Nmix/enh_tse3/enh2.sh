...
if ! "${skip_eval}"; then
            ...
            for use_true_nspk in true false; do
                ...
                for dset in ${test_sets}; do
                    ...
                    if [[ "${task}" == "tse" ]]; then
                        ...
                        infer_module=espnet2.bin.enh_variable_number_spkeakers_inference
                    fi
                    # shellcheck disable=SC2046,SC2086
                    ...
                    for spk in $(seq "${ref_num}"); do
                        for protocol in ${scoring_protocol}; do
                            if [ "${protocol}" != "Est_num_spk" ]; then
                                for i in $(seq "${_nj}"); do
                                    cat "${_scorelogdir}/output.${i}/${protocol}_spk${spk}"
    ...
    if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
        ...
        for dset in "${valid_set}" ${test_sets}; do
            ...
            if [ -e "${_dir}" ]; then
                log "Evaluation result already exists. The scoring for observation will be skipped"
                continue
            fi
            _logdir="${_dir}/logdir"
            _scoredir="${_dir}/scoring"
