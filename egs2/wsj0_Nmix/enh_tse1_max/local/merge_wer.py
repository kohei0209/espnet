...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "asr_result_root", type=Path
    )  # like "exp/enh_enh_dptnet_eda4thlayer_2-5mix_1bce_raw/enhanced_tt_max_8k/enh/5mix/without_true_numspk/whisper"
    ...
    for spk in range(1, args.n_mix + 1):
        ...
        with res.open("r") as f:
            for line in f:
                line = line.strip()
                match = re.fullmatch(
                    r"Scores: \(#C #S #D #I\)\s*(\d+)\s*(\d+)\s*(\d+)\s*(\d+)$",
                    line,
                )
                if not match:
                    continue
