...
for i in range(args.num_spk):
    with open(
        args.simulation_metadata_dir / f"simulation_metadata_{i+1}spk.json",
        "r",
    ) as f:
        m = json.load(f)
    ...
    assert (
        len(metadata) == original_length + new_length
    ), "Maybe some keys are duplicated and deleted"
# sort the keys
...
for audio_id, data in metadata.items():
    ...
    for i in range(args.num_spk):
        if i < true_num_spk:
            output_scps[f"spk{i+1}"].write(
                f"{audio_id} {anechoic_speech_paths[i]}\n"
            )
            output_scps[f"reverb_spk{i+1}"].write(
                f"{audio_id} {reverb_speech_paths[i]}\n"
            )
