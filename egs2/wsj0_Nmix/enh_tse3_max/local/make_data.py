...
def simulate_src(
    ...
    room = pra.ShoeBox(
        room_size, fs=fs, use_rand_ism=use_rand_ism, max_rand_disp=0.08,
        max_order=max_order, materials=pra.Material(e_absorption),
    )
    ...
    anechoic_room = pra.ShoeBox(
        room_size, fs=fs, use_rand_ism=use_rand_ism, max_rand_disp=0.08,
        max_order=0, materials=pra.Material(e_absorption),
    )
    # anechoic_room = pra.ShoeBox(room_size, fs=fs, max_order=0)
    anechoic_room.add_microphone_array(pra.MicrophoneArray(sensor_position, fs=anechoic_room.fs))
...
def run(
    ...
    if noise.shape[0] < drysrc.shape[0]:
        noise = np.pad(noise, (0, drysrc.shape[0] - noise.shape[0]), mode="wrap")
    ...
    weight = adjust_SNR(reverbs[inactive_source_idx], noise, snr=mixinfo['noise_snr'])
    reverbs *= weight
    anechoics *= weight
...
def make_src_data(args):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=n_thread) as excuter:
        list(tqdm(excuter.map(
            run,
            mix_info,
        ), total=len(mix_info)))
