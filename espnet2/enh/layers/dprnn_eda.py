...
# dual-path RNN
class DPRNN_EDA_Informed_Separated(nn.Module):
    """Deep dual-path RNN with encoder-decoder-based attractor.
    ...
    def __init__(
        ...
        adapt_layer_type: str = "mul",
        adapt_enroll_dim: int = 64,
        separate_output_side: bool = False,
    ):
        super().__init__()
        ...
        # if true, we use different DPRNN blocks for TSE and Enh
        self.separate_output_side = separate_output_side
        if separate_output_side:
            # dual-path RNN
            self.row_rnn_tse = nn.ModuleList([])
            self.col_rnn_tse = nn.ModuleList([])
            self.row_norm_tse = nn.ModuleList([])
            self.col_norm_tse = nn.ModuleList([])
            for i in range(num_layers - i_adapt_layer - 1):
                self.row_rnn_tse.append(
                    SingleRNN(
                        rnn_type, input_size, hidden_size, dropout, bidirectional=True
                    )
                )  # intra-segment RNN is always noncausal
                self.col_rnn_tse.append(
                    SingleRNN(
                        rnn_type,
                        input_size,
                        hidden_size,
                ...
                self.row_norm_tse.append(nn.GroupNorm(1, input_size, eps=1e-8))
                self.col_norm_tse.append(nn.GroupNorm(1, input_size, eps=1e-8))
    ...
    def forward(self, input, enroll_emb, num_spk=None):
        ...
        for i in range(len(self.row_rnn)):
            ...
            if self.separate_output_side and self.i_adapt_layer < i:
                row_output, _ = self.row_rnn_tse[i-self.i_adapt_layer-1](row_input)
            else:
                row_output, _ = self.row_rnn[i](row_input)  # B*dim2, dim1, H
            row_output = (
                row_output.view(batch_size, dim2, dim1, -1)
            ...
            if self.separate_output_side and self.i_adapt_layer < i:
                row_output = self.row_norm_tse[i-self.i_adapt_layer-1](row_output)
            else:
                row_output = self.row_norm[i](row_output)
            output = output + row_output
            ...
            if self.separate_output_side and self.i_adapt_layer < i:
                col_output, _ = self.col_rnn_tse[i-self.i_adapt_layer-1](col_input)
            else:
                col_output, _ = self.col_rnn[i](col_input)  # B*dim1, dim2, H
            col_output = (
                col_output.view(batch_size, dim1, dim2, -1)
            ...
            if self.separate_output_side and self.i_adapt_layer < i:
                col_output = self.col_norm_tse[i-self.i_adapt_layer-1](col_output)
            else:
                col_output = self.col_norm[i](col_output)
            output = output + col_output

            # compute attractor
            if is_tse and i == self.i_adapt_layer:
                assert num_spk == 1, f"num_spk = {num_spk}"
                output = output.reshape(-1, hidden_dim, dim1, dim2)
                if enroll_emb.ndim == 2:
                    enroll_emb = enroll_emb[..., None]
                ...
                output = self.adapt_layer(output, enroll_emb)
                probabilities = None
            elif not is_tse and i == self.i_eda_layer:
                aggregated_sequence = self.sequence_aggregation(output.transpose(-1, -3))
                attractors, probabilities = self.eda(aggregated_sequence, num_spk=num_spk)
                output = output[..., None, :, :, :] * attractors[..., :-1, :, None, None] # [B, J, N, L, K]
                output = output.reshape(-1, hidden_dim, dim1, dim2)
