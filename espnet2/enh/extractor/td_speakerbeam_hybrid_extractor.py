...
from espnet2.enh.layers.tcn import TemporalConvNet, TemporalConvNetInformedWithAttention
from espnet.nets.pytorch_backend.nets_utils import make_pad_mask


class TDSpeakerBeamExtractorWithAttention(AbsExtractor, AbsSeparator):
    def __init__(
        self,
        input_dim: int,
        layer: int = 8,
        ...
        self.tcn = TemporalConvNetInformedWithAttention(
            N=input_dim,
            B=bottleneck_dim,
            H=hidden_dim,
            P=kernel,
    ...
    def forward(
        ...
        masks = self.tcn(feature, enroll_emb)  # B, N, L
        # masks = masks.transpose(-1, -2)  # B, L, N
        # masked = input * masks

        masked = [input * m.transpose(-1, -2) for m in masks]
        if is_tse:
            masked = masked[0]
